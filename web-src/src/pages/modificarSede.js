...
const ModificarSede = () => {
  ...
  const idFormik = useFormik({
    ...
    validationSchema : Yup.object().shape({id:Yup.number().required("Ingresa un Id")})
  })
  const validationSchema = Yup.object().shape(
    {
      nombre : Yup.string().required("Nombre de Sede Obligatorio"),
      direccion : Yup.string().required("Dirección Obligatoria"),
      idAdministrador : Yup.number().required("Administrador Requerido")
    }
  )
